((name, definition) => {
    ...
    let
        iframe; let loaded;
    const secure = 'https:' == location.protocol || 'localhost' == location.hostname;
    const streamSaver = {
        createWriteStream,
        supported: false,
        version  : {
    ...
    try {
        ...
        streamSaver.supported = 'serviceWorker' in navigator && Boolean(new ReadableStream()) && Boolean(new WritableStream());
    } catch (err) {
